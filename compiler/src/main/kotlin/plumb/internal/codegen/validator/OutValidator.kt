...
object OutValidator : Validator {
    ...
    fun notEnclosedByPlumbedClass(element: Element)
            = "@Out-annotated Element ${element.simpleName} is not enclosed by @Plumbed class or class being plumbed to."
    ...
    override fun validate(element: Element, model: Model): Boolean {
        ...
        val joinModels = model.plumberEntries
        ...
        val joinModelsValid = validateJoinModels(element, joinModels)
    ...
    private fun validateField(element: Element): Boolean {
        return if (element.asType().kind == TypeKind.DECLARED) {
            ...
            if (!types.isAssignable(type,
                    types.getDeclaredType(elements.getTypeElement(observableQualifiedName),
    ...
    private fun validateMethod(element: Element): Boolean {
        ...
        val typeValid = if (executableType.kind == TypeKind.DECLARED) {
            val type = element.asType()
            if (!types.isAssignable(type,
                    types.getDeclaredType(elements.getTypeElement(observableQualifiedName),
                            types.getWildcardType(null, null)))) {
            ...
            if (returnType.toString() != observableQualifiedName) {
                messager.error(methodDoesNotReturnObservableError(element))
            }
            else {
                messager.error(fieldNotParameterizedObservable(element))
            }
    ...
    private fun validateJoinModels(element: Element, models: MutableList<PlumberModel>): Boolean {
        val entry = models
                .firstOrNull {
        ...
        if (entry == null) {
            messager.error(notEnclosedByPlumbedClass(element))
            return false
        }
